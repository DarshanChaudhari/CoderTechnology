...
/*
 * http://localhost:8080/GSManagement/appl/sm01/homePage
 * 
 * GSManagement - This is not project name, it is context root name, This can be view on properties 
 * and Web Project Setting
 * 
 */
...
public class SessionManage01 {
	...
	public String noSesssion(HttpServletRequest request){
		HttpSession session = request.getSession(false); /* This means if the session is there then is will return or
		 													if session is not there then it will not create
														 */				
		if (session == null){
	...
	public String createSession(HttpServletRequest request){
		HttpSession session = request.getSession(true);  // Manual way of creating the session.
															/* This means if the session is there then is will return or
																if session is not there then it will not create
														 */			
		if (session == null){
